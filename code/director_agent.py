...
class Api:
    """Video generation API client using public APIs.

    Each model produces independent output — you pick ONE model per run
    ...
    def call_data_eval(
        ...
        **kargs: Any,
        ...
        Return None on success (video saved to output_path).
        Return an error message string on failure.
        """
        os.makedirs(os.path.dirname(output_path), exist_ok=True)

        if model in self._VEO_MODELS or model.startswith("veo"):
            return self._call_veo(
                text, pic_path, pic_func, output_path, model, size, seconds,
            )
        if model in self._SORA_MODELS or model.startswith("sora"):
            return self._call_sora(
        ...
        if model in ("jimeng", "keling"):
            return f"Model '{model}' REST API not yet implemented."

        return f"Unknown model: {model}"

    # ------------------------------------------------------------------
    # Google Veo  (google-genai — already a project dependency)
    # ------------------------------------------------------------------
    ...
    def _call_veo(
        ...
        try:
            ...
            return "google-genai package required. Install: pip install google-genai"

        if not self.gemini_api_key:
            return (
                "Gemini API key required for Veo. "
                "Pass --gemini_api_key or set GEMINI_API_KEY."
            )

        client = genai.Client(api_key=self.gemini_api_key)

        api_model = self._VEO_MODELS.get(model)
        if not api_model:
            return f"Unknown Veo model variant: {model}"

        # -- Build config --
        config_kwargs: Dict[str, Any] = {}
        if size:
            config_kwargs["resolution"] = size  # "720p", "1080p", "4k"

        # -- Handle reference images --
        image_arg = None
        if pic_path and pic_func:
            from PIL import Image as PILImage

            reference_images = []
            for path, func in zip(pic_path, pic_func):
                img = PILImage.open(path)
                if func == "first":
                    image_arg = img
                elif func in ("style", "asset"):
        ...
        try:
            LOGGER.info("Calling Veo API (%s), prompt length: %d", api_model, len(text))

            gen_kwargs: Dict[str, Any] = {"model": api_model, "prompt": text}
            ...
            while not operation.done:
                if elapsed >= self.timeout:
                    return f"Veo generation timed out after {self.timeout}s"
            ...
            if not operation.response or not operation.response.generated_videos:
                return "Veo returned no video in response"

            video = operation.response.generated_videos[0]
            client.files.download(file=video.video)
            video.video.save(output_path)

            LOGGER.info("Veo video saved to %s", output_path)
            return None

        except Exception as exc:
            LOGGER.error("Veo API call failed: %s: %s", type(exc).__name__, exc)
            return str(exc)

    # ------------------------------------------------------------------
    # OpenAI Sora  (openai — already a project dependency)
    # ------------------------------------------------------------------
    ...
    def _call_sora(
        ...
        try:
            ...
            return "openai package required. Install: pip install openai"

        if not self.openai_api_key:
            return (
                "OpenAI API key required for Sora. "
                "Pass --openai_api_key or set OPENAI_API_KEY."
            )

        client = _openai.OpenAI(api_key=self.openai_api_key)

        api_model = self._SORA_MODELS.get(model)
        if not api_model:
            return f"Unknown Sora model variant: {model}"

        if pic_path:
            LOGGER.warning(
                "Sora public API does not support image-to-video. "
                "Reference images will be ignored for this node. "
        ...
        try:
            ...
            while True:
                if elapsed >= self.timeout:
                    return f"Sora generation timed out after {self.timeout}s"
                ...
                if job.status == "failed":
                    ...
                    return f"Sora generation failed: {error_msg}"
