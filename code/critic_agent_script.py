...
class OpenAIClient:
    """Client for calling OpenAI chat completion API."""
    ...
    def request(
        ...
        if isinstance(content_payload, list):
            text_parts = [p.get("value", "") for p in content_payload if p.get("type") == "text"]
            prompt_text = "\n".join(text_parts)
        else:
            prompt_text = content_payload
        ...
        try:
            ...
            LOGGER.error(f"OpenAI request failed: {type(e).__name__}: {e}")
            return None
